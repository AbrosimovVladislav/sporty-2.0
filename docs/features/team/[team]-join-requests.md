...
**Единственное место — главная команды `/team/[id]`**:
...
- Рендерится только когда реально загружены данные с `/api/teams/[id]/join-requests` — никаких заглушек/мерцаний из `pendingRequestsCount`

Из колокольчика-уведомлений (тип `team_join_request_received`) тап ведёт на `/team/[id]`, аккордеон сам подтянет состояние.

В настройках команды (`/team/[id]/settings`) и в `PageHeader` (шестерёнке) заявок нет — там только параметры команды.
