...
**Активные приглашения** — секция «Уже приглашён» (если есть pending team→player от моих команд): лого + название + «Отправлено N дней назад» + кнопка «Отозвать». Источник — `GET /api/players/[id]/invites?inviterId=`. После accept/reject автоматически исчезает из списка.

**Bottom action bar** «Пригласить в команду» — виден если просмотрщик не сам игрок и есть хотя бы одна команда-организатор без активного pending. В sheet выбора команды отфильтровано: команды с активным приглашением скрыты (нельзя дублировать).
