...
11. **Баннер «Событие прошло — завершить»** для организатора: показывается если `status=planned` и дата уже прошла. Жёлто-оранжевая карточка с кнопкой «Завершить», вызывает `PATCH status=completed`.
